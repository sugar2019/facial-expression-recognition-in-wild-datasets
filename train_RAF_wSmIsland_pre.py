...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default='model_RAF_pre_wSmIsland3', help='Pytorch checkpoint file path')
    ...
    parser.add_argument('--lambda1', type=float, default=0.1, help='weight of inter_class loss included in island loss .')
...
def weighted_island_loss(features, centers, targets, weight, lambda1):
    target_centers = centers[targets]
    target_weight = weight[targets]
    center_loss = torch.sum(target_weight * torch.sum((features - target_centers) ** 2, dim=1)) / 2.
    # print('center_loss:{}'.format(center_loss))

    pair_center_dot = torch.matmul(centers, centers.t())
    # print('pair_center_dot:{}'.format(pair_center_dot))

    center_norm_column = torch.norm(centers, dim=1, keepdim=True)
    # print('center_norm_column:{}'.format(center_norm_column))

    center_norm_row = center_norm_column.view(1, -1)

    pair_center_norm = torch.matmul(center_norm_column, center_norm_row)
    # print('pair_center_norm:{}'.format(pair_center_norm))

    mask = 1-torch.eye(centers.size(0)).to(device)
    # print('pair_center_dot*mask:{}'.format(pair_center_dot*mask))
    # print('pair_center_norm*mask:{}'.format(pair_center_norm*mask))
    # print('(pair_center_dot/pair_center_norm+1)*mask:{}'.format((pair_center_dot/pair_center_norm+1)*mask))
    inter_class_loss = torch.sum((pair_center_dot/pair_center_norm+1)*mask).to(device)/2.0
    # print('inter_class_loss:{}'.format(inter_class_loss))

    island_loss = center_loss + lambda1*inter_class_loss
    # print('center_loss:{}, inter+class_loss:{}'.format(center_loss, inter_class_loss))
    return island_loss
...
def get_center_delta(features, centers, targets, weight, lambda1):
    ...
    delta_inter_class = delta_inter_class_1-delta_inter_class_2.unsqueeze(1)*centers
    # print('delta_intra_centers:{};delta_inter_class:{};centers:{}'.format(delta_intra_centers.shape,delta_inter_class.shape,centers.shape))
    delta_centers = (delta_intra_centers + lambda1*delta_inter_class/(centers.size(0)-1))
...
def run_training():
    ...
    for i in range(1, args.epochs + 1):
        ...
        for imgs, targets, indexes in tqdm(train_loader):
            ...
            island_loss = weighted_island_loss(features, selfCenters, targets, weight, args.lambda1)
