...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default='model_RAF_pre_Self_sm1',
...
def softmax_loss(outputs,targets, classes, device):
    ...
    result = torch.sum(torch.log(softmax_outputs) *one_hot)
    return -result
...
def run_training():
    ...
    for i in range(1, args.epochs + 1):
        ...
        for imgs, targets, indexes in tqdm(train_loader):
            ...
            loss = softmax_loss(outputs, targets,classes=args.classes, device=device)
            loss.backward()
            optimizer.step()
