...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default='model_RAF_pre_Wsmce1', help='Pytorch checkpoint file path')
...
def weighted_center_loss(centers, features, targets, weight):
    target_centers = centers[targets]
    target_weight = weight[targets]
    loss = torch.sum(target_weight*torch.sum((features-target_centers)**2, dim=1))/2.
    return loss


def get_center_delta(features, centers, targets, weight):
    # implementation equation (4) in the center-loss paper
    # print("centers.type::", centers.dtype)
    features = features.view(features.size(0), -1)
    # print("targets:{}".format(targets))

    targets, indices = torch.sort(targets)

    # print("targets:{}, indices:{}".format(targets, indices))

    target_centers = centers[targets]  # 取centers的targets的升序排列
    features = features[indices]  # 对feature按照targets的升序排列

    delta_centers = target_centers - features  # 得到以targets的升序排列的delta

    uni_targets, indices = torch.unique(
        targets.cpu(), sorted=True, return_inverse=True)  # 得到独特的targets值和targets中的每个值在uni_targets中的位置
    uni_targets = uni_targets.to(device)
    indices = indices.to(device)

    # print("uni_targets:{}, indices:{}".format(uni_targets, indices))

    delta_centers = torch.zeros(
        uni_targets.size(0), delta_centers.size(1)
    ).to(device).index_add_(0, indices, delta_centers)  # 把属于同一个标签的delta加到同一个位置，但这里delta_centers的0维可能小于7

    targets_repeat_num = uni_targets.size()[0]
    uni_targets_repeat_num = targets.size()[0]
    targets_repeat = targets.repeat(
        targets_repeat_num).view(targets_repeat_num, -1)  # 得到uni_targets.size()[0] × batch_size
    uni_targets_repeat = uni_targets.unsqueeze(1).repeat(
        1, uni_targets_repeat_num) # 得到uni_targets.size()[0] × batch_size 与上面两个维度一样
    same_class_feature_count = torch.sum(
        targets_repeat == uni_targets_repeat, dim=1).float().unsqueeze(1)  # 看每个类各有几个样本

    delta_centers = delta_centers / (same_class_feature_count + 1.0)
    result = torch.zeros_like(centers)
    result[uni_targets, :] = delta_centers  # 将变化的类的center赋值给result
    result = weight.unsqueeze(1)*result
...
def run_training():
    ...
    for i in range(1, args.epochs + 1):
        ...
        for imgs, targets, indexes in tqdm(train_loader):
            ...
            center_loss = weighted_center_loss(selfCenters, features, targets, weight)

            loss = sm_loss + args.gamma * center_loss

            loss.backward()
            # print(centers.grad)
            center_deltas = get_center_delta(features, selfCenters, targets, weight)
