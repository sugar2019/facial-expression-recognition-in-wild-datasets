...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default='model_RAF_pre_wSmCl1_05', help='Pytorch checkpoint file path')
    parser.add_argument('--pretrained', type=str, default=True, help='Pretrained weights')

    parser.add_argument('--gamma', type=float, default=1.0, help='weight of center loss.')
...
def compute_dataset_weight(train_label_path, classes, device):
    ...
    weight = torch.tensor(min(ll)/np.array(list(ll))).to(device).type(torch.float32)
    return weight
...
def weighted_cluster(centers, features, targets, weight):
    target_centers = centers[targets]
    target_weight = weight[targets]
    intra_class = target_weight/2.0*torch.norm(features-target_centers, dim=1)**2

    inter_class = torch.sum(torch.norm(centers.unsqueeze(0)-target_centers.unsqueeze(1), dim=2)**2, dim=1)+1

    wCl = torch.sum(intra_class/inter_class)
    return wCl
def get_center_delta(centers, features, targets, weight):
    target_centers = centers[targets]
    target_weight = weight[targets]
    intra_class = target_weight / 2.0 * torch.norm(features - target_centers, dim=1) ** 2
    ...
    delta_centers = (target_centers-features)/inter_class.unsqueeze(1) + intra_class.unsqueeze(1)/(inter_class.unsqueeze(1)**2)*torch.sum(
        centers.unsqueeze(0)-target_centers.unsqueeze(1), dim=1)

    # 将center分布到centers中
    uni_targets, indices = torch.unique(targets, return_inverse=True)

    delta_centers = torch.zeros(uni_targets.size(0), centers.size(1)).to(device).index_add(0, indices, delta_centers)
    all_delta_centers = torch.zeros_like(centers)
    all_delta_centers[uni_targets, :] = delta_centers
    all_delta_centers = weight.unsqueeze(1)*all_delta_centers
    all_delta_centers = all_delta_centers.detach().type(torch.float32)
    return all_delta_centers
...
def run_training():
    ...
    for i in range(1, args.epochs + 1):
        ...
        for imgs, targets, indexes in tqdm(train_loader):
            ...
            centers = res18.centers
            center_loss = weighted_cluster(centers, features, targets, weight)

            loss = sm_loss + args.gamma*center_loss
            loss.backward()

            center_deltas = get_center_delta(centers, features, targets, weight)
            # res18.centers = centers-center_deltas
            res18.register_buffer("centers", centers-center_deltas)
