...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default='model_RAF_pre_wSmLce+1', help='Pytorch checkpoint file path')
...
def weighted_LCE_loss(features, centers, targets, weight, lambda1):
    n = features.size(0)
    target_centers = centers[targets]
    target_weight = weight[targets]
    L2L = torch.sum(target_weight * torch.sum((features - target_centers) ** 2, dim=1)) / 2.

    CDL = compute_CDL(centers)
    EDL = compute_EDL(targets, centers, n)
    # print('CDL:{}, \n EDL:{}'.format(CDL,EDL))

    LCE_loss = L2L + lambda1*CDL/EDL
    # print('center_loss:{}, inter+class_loss:{}, CDL:{}, EDL:{}'.format(L2L, CDL/EDL, CDL, EDL))

    return LCE_loss

def get_center_delta(features, centers, targets, weight, lambda1):
    n = features.size(0)
    # compute delta intra class center
    targets, indices = torch.sort(targets)
    target_center = centers[targets]
    features = features[indices]
    delta_intra_class = target_center - features

    t_unique, indices = torch.unique(targets, return_inverse=True)
    delta_intra_class = torch.zeros(t_unique.size(0), delta_intra_class.size(1)).to(device). \
        index_add_(0, indices, delta_intra_class)
    # target_repeat_num = t_unique.size()[0]
    # t_unique_repeat_num = targets.size()[0]
    # targets_repeat = targets.repeat(target_repeat_num).reshape(target_repeat_num, -1)
    # t_unique_repeat = t_unique.unsqueeze(1).repeat(1, t_unique_repeat_num)
    # same_class_num = torch.sum(torch.eq(targets_repeat, t_unique_repeat), 1).float().unsqueeze(1)
    #
    # delta_intra_class = delta_intra_class / (same_class_num + 1.0)
    delta_intra_centers = torch.zeros_like(centers)
    delta_intra_centers[t_unique, :] = delta_intra_class
    delta_intra_centers = weight.unsqueeze(1) * delta_intra_centers
    delta_intra_centers = delta_intra_centers.detach().type(torch.float32)

    # print('delta_intra_centers:{}'.format(delta_intra_centers))
    # compute delta of CDL and EDL
    delta_inter_center = get_CDL_center_delta(centers) / compute_EDL(targets, centers, n) + \
                         get_EDL_center_delta(targets, centers, n) * compute_CDL(centers)
    # print('get_EDL_center_delta(targets, centers, n):{}'.format(get_EDL_center_delta(targets, centers, n)))

    # print('delta_inter_center:{}'.format(delta_inter_center))


    delta_center = delta_intra_centers + lambda1 / (centers.size(0) - 1) * delta_inter_center

    # print('delta_center:{}'.format(delta_center))
    return delta_center

def compute_EDL(targets, centers, n):
    uni_target = torch.unique(targets)
    ...
    # same_class_ratio = same_class_num/n
    # all_same_class_ratio = torch.zeros(centers.size(0), 1).to(device)
    # all_same_class_ratio[uni_target, :] = same_class_ratio
    # pair_same_class = torch.matmul(all_same_class_ratio, all_same_class_ratio.t())

    pair_center_sub = centers.unsqueeze(1)-centers.unsqueeze(0)
    pair_center_sub_norm = torch.norm(pair_center_sub, dim=2)
    # EDL = 1.0/2*torch.sum(pair_same_class*pair_center_sub_norm)
    EDL = 1.0 / 2 * torch.sum(pair_center_sub_norm) + 1.
    return EDL
...
def get_EDL_center_delta(targets, centers, n):
    uni_target = torch.unique(targets)
    uni_target_repeat_num = targets.size()[0]
    target_repeat_num = uni_target.size()[0]
    ...
    mask = torch.eye(centers.size(0)).to(device)

    # delta_EDL_center = pair_center_sub/(mask+pair_center_sub_norm).unsqueeze(2)*pair_same_class.unsqueeze(2)
    delta_EDL_center = pair_center_sub/(mask+pair_center_sub_norm).unsqueeze(2)

    delta_EDL_center = torch.sum(delta_EDL_center, dim=(0, 1))/2.0
    deno = compute_EDL(targets, centers, n)**2
    delta_EDL_center = -delta_EDL_center/deno
    return delta_EDL_center
...
def run_training():
    ...
    for i in range(1, args.epochs + 1):
        ...
        for imgs, targets, indexes in tqdm(train_loader):
            ...
            island_loss = weighted_LCE_loss(features, selfCenters, targets, weight, args.lambda1)

            loss = sm_loss + args.gamma * island_loss
